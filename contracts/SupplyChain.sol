...
contract SupplyChain {
    ...
    struct rawMaterialSupplier {
        ...
        string phone; //Phone number of the raw material supplier
        string role; //Role of the raw material supplier
    }
    ...
    struct manufacturer {
        ...
        string phone; //Phone number of the manufacturer
        string role; //Role of the manufacturer
    }
    ...
    struct distributor {
        ...
        string phone; //Phone number of the distributor
        string role; //Role of the distributor
    }
    ...
    struct retailer {
        ...
        string phone; //Phone number of the retailer
        string role; //Role of the retailer
    }
    ...
    struct consumer {
        uint256 id; // unique consumer id
        string name; // Name of the consumer
        string phone; // Phone number of the consumer
        uint256 medicineID; // ID of the medicine bought by the consumer
        address retailerAddress; // Address of the retailer who sold the medicine
    }

    // Mapping to store consumer data with the unique ID
    mapping(uint256 => consumer) public Consumers;

    // Counter for unique consumer ID
    ...
    function addRetailer(
        ...
        retCtr++;
        RET[retCtr] = retailer(_address, retCtr, _name, _place, _phone, _role);
    }
    ...
    function sold(
        ...
        // Increment consumer counter and create a new consumer entry
        consumerCtr++;
    ...
    function getConsumer(
        ...
        returns (
            ...
            string memory consumerPhone,
            uint256 medicineID,
            address retailerAddress
        )
    {
        require(_consumerID > 0 && _consumerID <= consumerCtr);

        consumer storage selectedConsumer = Consumers[_consumerID];

        return (
            selectedConsumer.name,
            selectedConsumer.phone,
            selectedConsumer.medicineID,
            selectedConsumer.retailerAddress
        );
    }

    function getConsumersByRetailer(
        address _retailerAddress
    ) public view returns (uint256[] memory consumerIDs) {
        uint256 count = 0;

        for (uint256 i = 1; i <= consumerCtr; i++) {
            if (Consumers[i].retailerAddress == _retailerAddress) {
                count++;
            }
        }

        consumerIDs = new uint256[](count);
    ...
    {
        uint256 rawSupplierID = findUserAddressInRole(
            _userAddress,
            "rawsupplier"
        );
        if (rawSupplierID > 0) {
            return (
                RMS[rawSupplierID].name,
                RMS[rawSupplierID].place,
                RMS[rawSupplierID].phone,
                RMS[rawSupplierID].role
            );
        }

        uint256 retailerID = findUserAddressInRole(_userAddress, "retailer");
        if (retailerID > 0) {
            return (
                RET[retailerID].name,
                RET[retailerID].place,
                RET[retailerID].phone,
                RET[retailerID].role
            );
        }

        uint256 distributorID = findUserAddressInRole(
            _userAddress,
            "distributor"
        );
        if (distributorID > 0) {
            return (
                DIS[distributorID].name,
                DIS[distributorID].place,
                DIS[distributorID].phone,
                DIS[distributorID].role
            );
        }

        uint256 manufacturerID = findUserAddressInRole(
            _userAddress,
            "manufacturer"
        );
        if (manufacturerID > 0) {
            return (
                MAN[manufacturerID].name,
                MAN[manufacturerID].place,
                MAN[manufacturerID].phone,
                MAN[manufacturerID].role
            );
        }

        // Add similar checks for other roles

        // Return empty strings if the address doesn't match any role
        return ("", "", "", "");
    }

    // Internal function to find a user's ID in a specific role by address
    function findUserAddressInRole(
        address _userAddress,
        string memory _role
        ...
        if (
            keccak256(abi.encodePacked(_role)) ==
            keccak256(abi.encodePacked("rawsupplier"))
        ) {
            for (uint256 i = 1; i <= rmsCtr; i++) {
                if (RMS[i].addr == _userAddress) {
                    return i;
                }
            }
        }
        ...
        // Return 0 if the address doesn't match any role
        return 0;
    }
}
